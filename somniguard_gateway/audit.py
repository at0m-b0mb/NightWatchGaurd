...
class AuditLogger:
    ...
    def _emit(self, event_type: str, ip_address: str,
        ...
        """
        Build a structured audit record and write it to all three sinks
        (console, rotating JSON file, and the audit_log SQL table).
        ...
        # 1. console + rotating JSON file
        self._logger.info(json_line)

        # 2. SQL audit_log table — used by the dashboard viewer + CSV export.
        #    Best-effort: a DB outage must never silently drop the audit
        #    record (the JSON file is still authoritative).
        try:
            ...
            print("[SOMNI][AUDIT][WARN] DB sink failed: {}".format(exc))
