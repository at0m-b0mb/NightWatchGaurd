...
# ticks_ms is MicroPython‑specific; provide a CPython fallback for
...
def format_reading(sensor_data):
    ...
    # GSR
    gsr_d  = sensor_data.get("gsr", {})
    gsr_us = gsr_d.get("conductance_us")
    gsr_str = "{:.2f}uS".format(gsr_us) if gsr_us is not None else "---"

    return (
        "t={ms}ms SpO2={spo2} HR={hr} accel={accel} GSR={gsr}".format(
            ms=ts,
            spo2=spo2_str,
            hr=hr_str,
            accel=accel_str,
