...
_flush_requested = False  # set by timer callback; consumed by idle loop
...
def _on_sensor_data(data):
    ...
    - When the batch is full, sets _flush_requested so the idle loop
      flushes outside the timer callback (avoids blocking sensor sampling
      and watchdog starvation during long HTTP I/O).
    ...
    Returns:
        None
    """
    global _pending_batch, _session_id, _flush_requested
    ...
    try:
        ...
        if len(_pending_batch) >= config.TRANSPORT_BATCH_SIZE:
            # Signal the idle loop to flush — do NOT flush here.
            # Calling blocking socket I/O from inside the timer callback
            # prevents the timer from firing again for the entire duration
            # of the HTTP requests, causing missed sensor samples and
            # watchdog starvation.
...
def _flush_batch():
    ...
    The watchdog is fed between requests so a slow network cannot
    trigger a reset mid-flush.
    ...
    for reading in batch:
        _feed_watchdog()  # keep WDT alive across multiple HTTP requests
        try:
...
def _run_integrity_check():
    ...
    try:
        ...
        if passed:
            ...
            print("[SOMNI][INTEGRITY] *** FATAL: Integrity check FAILED ***")
            print("[SOMNI][INTEGRITY] Tampered or corrupted firmware detected.")
            raise RuntimeError(
                "Firmware integrity check failed — halting to prevent "
...
def main():
    ...
    """
    global _led, _wdt, _session_id, _flush_requested
    ...
    if config.TRANSPORT_ENABLED:
        # Print the HMAC key fingerprint (sha256[:8]) so it can be visually
        # compared against the gateway's startup log. The two values MUST
        # match — if they differ, the gateway will reject every /api/* call
        # with "HMAC verification failed".  MicroPython's hashlib.sha256
        # only exposes .digest() (no .hexdigest()), so we hex-encode the
        # bytes manually.
        try:
            import hashlib as _hashlib
            _hk = config.GATEWAY_HMAC_KEY
            _hk_bytes = _hk.encode("utf-8") if isinstance(_hk, str) else _hk
            _digest = _hashlib.sha256(_hk_bytes).digest()
            _fp = "".join("{:02x}".format(b) for b in _digest)[:8]
            print("[SOMNI][HMAC] GATEWAY_HMAC_KEY sha256[:8]={} len={}  "
                  "— must match gateway's [SOMNI][CONFIG] line.".format(_fp, len(_hk)))
        except Exception as _hk_exc:
            print("[SOMNI][HMAC] could not fingerprint key: {}".format(_hk_exc))
        _wifi_retries = getattr(config, "WIFI_MAX_RETRIES", 5)
        ip = transport.connect_wifi_with_retries(
            config.WIFI_SSID,
            config.WIFI_PASSWORD,
            max_retries=_wifi_retries,
            timeout_s=config.WIFI_CONNECT_TIMEOUT_S,
            feed_wdt=_feed_watchdog,
        )
        _feed_watchdog()
        if ip:
            print("[SOMNI] Connected to Wi‑Fi as {}.".format(ip))

            # Sync clock so Unix timestamps are correct for gateway freshness checks.
            # MicroPython's time.time() counts from 2000-01-01; transport.py adds
            # _EPOCH_OFFSET to convert to Unix epoch. NTP is the accurate path;
            # gateway /api/time is the fallback when the hotspot has no internet.
            _clock_synced = False
            try:
                import ntptime
                ntptime.settime()
                print("[SOMNI] NTP clock sync successful.")
                _clock_synced = True
            except Exception as _ntp_exc:
                print("[SOMNI][WARN] NTP sync failed ({}); "
                      "trying gateway time…".format(_ntp_exc))

            if not _clock_synced:
                _clock_synced = transport.sync_time_from_gateway(
                    config.GATEWAY_HOST,
                    config.GATEWAY_PORT,
                )
                if not _clock_synced:
                    print("[SOMNI][WARN] Clock sync unavailable; "
                          "ingest timestamps may be rejected by gateway.")
    ...
    # ---------------------------------------------------------------
    # 7. Idle loop — keep main thread alive; flush batches here so
    #    the timer callback stays fast and the watchdog is always fed.
    # ---------------------------------------------------------------
    _SESSION_RETRY_INTERVAL_S = 30   # retry a failed session start every 30 s
    _last_session_retry = 0
    ...
    try:
        while True:
            ...
            # Drain any batch flagged by the timer callback.
            # Flushing here (not inside the callback) keeps the timer ISR
            # fast and prevents watchdog starvation during HTTP I/O.
            if _flush_requested and _session_id is not None:
                _flush_requested = False
                _flush_batch()
                _feed_watchdog()

            # Retry session start if it failed at boot or was lost
            if _session_id is None and config.TRANSPORT_ENABLED:
                now = int(time.time())
                if now - _last_session_retry >= _SESSION_RETRY_INTERVAL_S:
                    _last_session_retry = now
                    print("[SOMNI] Retrying gateway session start…")
                    _session_id = transport.start_session(
                        config.GATEWAY_HOST,
                        config.GATEWAY_PORT,
                        config.GATEWAY_PATIENT_ID,
                    ...
                    if _session_id:
                        print("[SOMNI] Gateway session started: "
                              "ID {}.".format(_session_id))

            # Short sleep so the timer callback can fire and the WDT is fed
            # well within its 8 s timeout even during session-retry I/O.
            time.sleep_ms(100)
