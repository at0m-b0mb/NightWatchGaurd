...
> This PHA is produced by Team NightWatchLabs as a research exercise
> aligned with IEC 62304 / ISO 14971 risk‑analysis concepts.  Severity
> and likelihood scores are educational estimates only.
...
### H-08: Firmware integrity compromise leading to false readings

| Field | Detail |
|-------|--------|
| **Hazard ID** | H-08 |
| **Hazard** | Tampered firmware produces false SpO₂/HR values that appear valid, leading to misdiagnosis |
| **Cause chain** | Attack path G5.1 (physical firmware replacement) or G5.3 (integrity bypass) → modified firmware reports normal SpO₂ during actual desaturation events → gateway accepts readings as valid (HMAC key compromised or integrity check bypassed) → report shows no apnea → clinician under-treats patient |
| **Assets affected** | A1 (Pico firmware), A5 (sensors), A7 (integrity manifest) |
| **Severity (S)** | 3 — Critical (under-treatment of sleep apnea can lead to cardiovascular events) |
| **Likelihood (L)** | 1 — Remote (requires physical access to Pico AND knowledge of HMAC key to forge manifest) |
| **Risk (R)** | 3 — Low |
| **Mitigations** | (1) SHA-256 integrity manifest verified at every boot (integrity.py). (2) Manifest signed with HMAC-SHA256 — attacker must know the key to forge. (3) Encrypted configuration storage prevents HMAC key extraction from filesystem. (4) Hardware-derived encryption key requires physical access to the specific Pico. |

---

### H-09: Unauthorized firmware modification

| Field | Detail |
|-------|--------|
| **Hazard ID** | H-09 |
| **Hazard** | Gateway Python modules modified to alter report generation or disable security controls |
| **Cause chain** | Attack path G5.2 (modify gateway modules) → attacker changes reports.py thresholds or disables HMAC verification in app.py → system accepts injected telemetry or produces misleading reports |
| **Assets affected** | A2 (Pi 5), A4 (data), A8 (secure boot chain) |
| **Severity (S)** | 3 — Critical (potential for misdiagnosis if report logic is altered) |
| **Likelihood (L)** | 1 — Remote (requires OS-level access to Pi 5; mitigated by Secure Boot and least-privilege) |
| **Risk (R)** | 3 — Low |
| **Mitigations** | (1) Pi 5 UEFI Secure Boot ensures only signed kernels/bootloaders execute. (2) Least-privilege service user prevents unauthorized file modification. (3) LUKS2 disk encryption protects against offline modification. (4) Audit logging detects suspicious access patterns. |
