...
| Gateway   | Raspberry Pi 5 | Data collection, storage, processing, web UI; Secure Boot (UEFI) |
...
│  [MAX30102]──┐                                                           │
│  (SpO2/HR)   │                                                           │
│              ├──I2C──► [Integrity check / integrity.py]                  │
│  [ADXL345]───┘              │                                            │
│  (Accel)                    ▼                                            │
│                        [Firmware / sampler.py]                           │
│                             │  Tagged telemetry packets                  │
│                             │  (timestamp, SpO2, HR, accel, GSR,        │
│  [GSR sensor]──ADC──────────┘   validity flags)                         │
│                             │                                            │
│  [Encrypted config / secure_config.py]  <- key storage                  │
│                             │                                            │
│                             ▼                                            │
...
│  └───────────────────────────────┘                                       │
│                             │                                            │
│  [Flask web server / app.py -- TLS/HTTPS]                               │
...
│      * Rate limiting, security headers, password policy, lockout        │
│                             │                                            │
│  [Audit logging / audit.py]                                              │
│      * Structured JSON audit trail for all security-relevant events     │
│                             │                                            │
│  [Tailscale daemon]         │  Secure Boot (UEFI)                       │
...
|--------|-----------|-----|------|
| MAX30102 (SpO2/HR) | I2C0 @ 400 kHz | SDA=GP4, SCL=GP5 | 1 Hz |
| ADXL345 (Accel) | I2C1 @ 400 kHz | SDA=GP2, SCL=GP3 | 10 Hz |
...
somniguard_pico/
  ...
  integrity.py     <- SHA-256 manifest-based firmware integrity verification
  secure_config.py <- XTEA-encrypted config storage (keys derived from HW ID)
  drivers/
    __init__.py
...
- Firmware integrity is verified at boot against a signed manifest.
- Sensitive configuration (HMAC keys, WiFi credentials) encrypted at rest.
- Anti-replay nonces and timestamps on every HMAC-signed packet.
- Hardware watchdog timer (8s timeout) resets device on firmware hang.
...
| Security middleware | security.py -- rate limiting, headers, password policy, lockout |
| Audit logging | audit.py -- structured JSON audit trail |
| TLS certificates | tls_setup.py -- self-signed cert generation |
| Secure Boot | UEFI Secure Boot via setup_secure_boot_pi5.sh |
...
somniguard_gateway/
  ...
  security.py      <- rate limiting, security headers, password policy, lockout
  audit.py         <- structured audit logging with rotation
  tls_setup.py     <- TLS certificate generation and management
...
| TB2 | Pico <-> Pi 5 (local LAN) | Network interception, replay, DoS | HMAC-SHA256 with anti-replay nonces + timestamp validation on every packet |
...
| TB6 | Pico filesystem | Firmware tampering, config theft | SHA-256 integrity manifest, XTEA-encrypted config, hardware-derived keys |
...
5. **Hardware security**: RP2350 watchdog timer provides crash recovery. Firmware integrity is verified via SHA-256 manifest at boot. Pi 5 supports UEFI Secure Boot.
