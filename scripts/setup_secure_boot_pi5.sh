...
# Raspberry Pi 5 running the pftf/RPi5 UEFI firmware. Secure Boot ensures
...
check_pi5() {
    ...
    if [[ ! -d "$EFI_MOUNT" ]]; then
        ...
        warn "If you have not yet installed the pftf/RPi5 UEFI firmware, this"
...
sign_boot_chain() {
    ...
    for bl in "${bootloader_candidates[@]}"; do
        if [[ -f "$bl" ]]; then
            ...
            run cp "${bl}.signed" "$bl"
            info "Bootloader signed and replaced in-place."
        fi
    done
    ...
    if [[ -f "$shell_path" ]]; then
        ...
        _sign_efi_binary "$shell_path" "${shell_path}"
    fi
}
...
configure_eeprom() {
    ...
    if $DRY_RUN; then
        ...
        info "[DRY-RUN] Would set BOOT_ORDER=0xf41 (SD→USB→NVMe→restart)"
    else
        ...
        # 0xf41: try SD (1), then USB (4), then restart (f).
        # Adjust to your hardware layout (e.g., add NVMe = 6).
        ...
        if grep -q "^BOOT_ORDER" "$tmp_cfg"; then
            sed -i 's/^BOOT_ORDER=.*/BOOT_ORDER=0xf16/' "$tmp_cfg"
        else
            echo "BOOT_ORDER=0xf16" >> "$tmp_cfg"
        fi

        # 0xf16: NVMe (6) → SD (1) → restart (f)
        # Change this to match your actual boot device priority.
...
# ---------------------------------------------------------------------------
#
# The pftf/RPi5 project provides a UEFI firmware image (RPI_EFI.fd) and
# associated files that replace the standard Raspberry Pi firmware on the
# FAT32 boot partition. This unlocks UEFI features including Secure Boot.
#
# Project: https://github.com/pftf/RPi5

install_uefi_firmware() {
    step "5. Installing pftf/RPi5 UEFI firmware"
    ...
    for f in config.txt fixup*.dat start*.elf bcm2712-rpi-5-b.dtb; do
        [[ -f "${boot_partition}/${f}" ]] && run cp "${boot_partition}/${f}" "$BACKUP_DIR/boot/" || true
    done
...
#    a. Download a fresh copy of the pftf/RPi5 firmware zip.
